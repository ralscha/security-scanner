...
Linear publication returns `0` after a completed batch run even when individual
findings were rejected; inspect the created/failed counts or `--json` result.
Configuration, handoff, receipt-without-success, and transport setup failures
return `2`. An interrupted publication returns `130`/`143` after saving a
partial receipt and retaining its private recovery handoff when possible.
